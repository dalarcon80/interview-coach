...
type AppTab = "prepare" | "coach" | "live" | "settings";
...
function App() {
  ...
  // History count for suggestions (default: 4, range: 1-20)
  ...
  useEffect(() => {
    ...
    setContextPersisted(hasPersistedContext());
  }, [backendUrl, candidateProfile, companyInfo, cvText, language, selectedStyle]);
  ...
  const submitCoachQuestion = useCallback(
    ...
    [backendMode, candidateProfile, companyInfo, cvText, language, liveSession.sessionId, selectedStyle]
  );

  // Handler for manual "Get Suggestions" button
  // Sends session_id to backend, which automatically reads last 4 messages from conversation history
  const handleGetSuggestions = useCallback(async () => {
    ...
    // Build question text from interviewer transcripts only (most recent)
    const interviewerTranscripts = transcriptsToUse.filter(t => t.speaker === 'interviewer');
    const lastInterviewer = interviewerTranscripts.length > 0 
      ? interviewerTranscripts[interviewerTranscripts.length - 1] 
      : null;
    const lastInterviewerText = lastInterviewer?.text ?? '';
    
    // If no interviewer text, use all text joined
    const questionText = lastInterviewerText || transcriptsToUse.map(t => t.text).join(' ');
    ...
    console.log('[DEBUG][Get Suggestions] Question text (from interviewer):', lastInterviewerText);
    ...
    const requestInfo = {
      ...
      max_words: companyInfo?.max_words || 200, // NEW: Length control
    };
  ...
  }, [liveSession.sessionId, candidateProfile, companyInfo, selectedStyle, language, backendMode, historyCount]);
  ...
  const startLiveSession = useCallback(() => {
    ...
    if (!effectiveCompany.name.trim() || !effectiveCompany.role_title.trim()) {
      setLiveError("Company name and role title are required to start live session");
      return;
    }
    ...
    sendWs("start_session", {
      config: {
        ...
        candidate: {
          ...
          currentRole: effectiveCandidate.current_role || undefined,
          yearsExperience: effectiveCandidate.years_experience || undefined,
          summary: effectiveCandidate.summary || undefined,
          skills: effectiveCandidate.skills,
          achievements: effectiveCandidate.achievements,
              ...
              <div className="space-y-4 p-4">
                ...
                <div className="grid grid-cols-1 gap-4 xl:grid-cols-12">
                  <section className="xl:col-span-4">
                    <CVIntake onProfileExtracted={handleCvProfileExtracted} />
                  </section>
                  <section className="xl:col-span-4">
                    <CandidateProfileForm
                      profile={effectiveCandidate}
                      onChange={(next) => setCandidateProfile(next)}
                    />
                  </section>
            ...
            <ScrollArea className="h-[calc(100vh-190px)] rounded-md border">
              <div className="space-y-4 p-4">
                ...
                <div className="grid grid-cols-1 gap-4 xl:grid-cols-12">
                            ...
                            {debugInfo && (
                              <div className="mt-3 rounded-md border border-amber-300 bg-amber-50 p-3">
                                <div className="flex items-center gap-2 mb-2">
                                  ...
                                  <span className="text-xs font-bold text-amber-700 uppercase">Debug: What was sent to coach</span>
                                </div>
                                <div className="space-y-2 text-xs">
                                    ...
                                    return (
                                      <>
                                        ...
                                        {history && history.length > 0 && (
                                          <div className="rounded bg-white border border-amber-200">
                                            <div className="p-2 bg-amber-100 font-semibold text-amber-800">
                                              📋 Questions/Messages Sent to Coach ({history.length}):
                                            </div>
                                            <div className="max-h-48 overflow-y-auto p-2 space-y-2">
                                              {history.map((msg, idx) => (
                                                <div key={idx} className="border-l-2 border-amber-300 pl-2">
                                                  <span className={msg.speaker === 'interviewer' ? 'text-blue-600 font-bold' : 'text-green-600 font-bold'}>
                                  ...
                                  <details className="rounded bg-white">
                                    <summary className="cursor-pointer p-2 font-semibold text-amber-800 hover:bg-amber-100">View Full Request JSON</summary>
                                    <pre className="max-h-40 overflow-auto p-2 text-[10px] bg-slate-100">
                                  ...
                                  {debugInfo.response && (
                                    <details className="rounded bg-white">
                                      <summary className="cursor-pointer p-2 font-semibold text-amber-800 hover:bg-amber-100">View Full Response JSON</summary>
                                      <pre className="max-h-40 overflow-auto p-2 text-[10px] bg-slate-100">
                  ...
                  {/* DUAL_STT_PHASE3: Live Captions - Zoom/Teams-like real-time display */}
                  <section className="space-y-4 xl:col-span-4">
                    <Card>
                      <CardHeader className="pb-2">
                        ...
                        <CardDescription>Real-time transcription as you speak</CardDescription>
                      </CardHeader>
                      <CardContent>
                            ...
                            <div className="space-y-3">
                              {Object.entries(liveCaptions).map(([speaker, caption]) => {
                                ...
                                const speakerColorClass =
                                  speaker === "interviewer" ? "text-blue-600" :
                                  speaker === "candidate" ? "text-emerald-600" :
                                  "text-purple-600";
                                return (
                          ...
                          {liveTranscripts.length === 0 ? (
                            ...
                            <div className="space-y-2">
                              {liveTranscripts.map((entry) => {
                                ...
                                const speakerColorClass =
                                  entry.speaker === "interviewer" ? "text-blue-600" :
                                  entry.speaker === "candidate" ? "text-emerald-600" :
                                  entry.speaker === "system" ? "text-purple-600" : "text-slate-600";
                  ...
                  <section className="space-y-4 xl:col-span-5">
                    <Card>
                      <CardHeader className="pb-2">
                        ...
                        <CardDescription>
                          Bullets arrive first as a preview. Full response replaces them as the primary output.
                        </CardDescription>
                      </CardHeader>
                      <CardContent className="space-y-3">
                        ...
                        {liveProcessing && liveSuggestion && !hasFullResponse && (
                          <Alert>
                            ...
                            <AlertDescription>Preview bullets are visible while the full response is composed.</AlertDescription>
                          </Alert>
                        )}
                        ...
                        {!liveSuggestion ? (
                          ...
                          <>
                            ...
                            >
                              ...
                              {hasFullResponse ? (
                                <p className="whitespace-pre-wrap text-sm leading-6 transition-opacity duration-300 md:text-base">
                                  {liveSuggestion.fullResponse}
                                </p>
                              ) : (
                                <div className="space-y-2">
                                      ...
                                      <div className="space-y-3 pt-2 text-xs">
                                        ...
                                        <div className="flex items-center gap-2">
                                          <Badge variant="outline" className="text-[10px]">
                                            History Count: {liveSuggestion.debug.history_count}
                                          </Badge>
                                          <Badge variant="outline" className="text-[10px]">
                                            Messages: {liveSuggestion.debug.conversation_history.length}
                                          </Badge>
                                        </div>

                                        {/* Conversation History */}
                                        <div className="space-y-1">
                                          <p className="font-semibold text-amber-800">Conversation History:</p>
                                          <div className="max-h-32 overflow-y-auto rounded bg-white p-2">
                                            {liveSuggestion.debug.conversation_history.map((msg, idx) => (
                                              <div key={idx} className="mb-1">
                                                <span className={msg.speaker === 'interviewer' ? 'text-blue-600 font-semibold' : 'text-green-600 font-semibold'}>
                                        ...
                                        {/* Question */}
                                        <div className="space-y-1">
                                          <p className="font-semibold text-amber-800">Question:</p>
                                          <p className="rounded bg-white p-2 text-gray-700">{liveSuggestion.debug.question}</p>
                                        </div>

                                        {/* Prompts */}
                                        <div className="space-y-1">
                                          <p className="font-semibold text-amber-800">System Prompt (first 500 chars):</p>
                                          <pre className="max-h-24 overflow-y-auto rounded bg-slate-100 p-2 text-[10px]">
                                            {liveSuggestion.debug.system_prompt?.slice(0, 500)}...
                                          </pre>
                                        </div>

                                        <div className="space-y-1">
                                          <p className="font-semibold text-amber-800">User Prompt (first 1000 chars):</p>
                                          <pre className="max-h-32 overflow-y-auto rounded bg-slate-100 p-2 text-[10px]">
                                            {liveSuggestion.debug.user_prompt?.slice(0, 1000)}...
                                          </pre>
                                        </div>
                                      </div>
                                    </AccordionContent>
